...
Toàn bộ code của bài nằm trong file `ingestion.py` (tham khảo từ repo chính thức của khóa học) — hàm `main()` chính là phần TavilyCrawl của bài (file còn import helper `logger.py` của project):
