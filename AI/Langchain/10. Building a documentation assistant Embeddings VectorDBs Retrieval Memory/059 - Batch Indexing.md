...
Toàn bộ code của bài nằm trong file `ingestion.py` (tham khảo từ repo chính thức của khóa học) — coroutine `index_documents_async` chính là phần batch indexing của bài (file còn import helper `logger.py` của project):
