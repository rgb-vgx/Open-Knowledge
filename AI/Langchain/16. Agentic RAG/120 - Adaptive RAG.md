...
### 💻 Code mẫu đầy đủ — `router.py`

Toàn bộ code của bài nằm trong file `graph/chains/router.py` (tham khảo từ repo chính thức của khóa học; repo đặt field là `datasource` với hai giá trị `vectorstore`/`websearch`):
