...
> Ghi chú cập nhật: theo Docker Compose Specification hiện tại, trường top-level `version` chỉ còn mang tính tương thích ngược và có thể tạo cảnh báo obsolete. Với Compose v2, bạn nên bỏ `version` và để Compose tự dùng schema mới nhất.
...
### Kiến trúc image theo chuẩn OCI

Docker image hiện đại nên được hiểu là một **OCI image**: gồm manifest, config và các filesystem layers bất biến. Khi container chạy, Docker/containerd thêm một writable layer mỏng phía trên các read-only layers. Các ghi chép phát sinh trong writable layer sẽ mất khi container bị xóa, trừ khi bạn mount volume hoặc bind mount ra ngoài.

```mermaid
flowchart TB
    subgraph "OCI Image (read-only)"
        M[Manifest<br/>digest, media types]
        C[Config<br/>env, cmd, entrypoint]
        L3[App layer<br/>COPY source]
        L2[Dependency layer<br/>RUN install]
        L1[Base OS/runtime layer<br/>FROM]
    end

    subgraph "Container Runtime"
        W[Writable container layer<br/>copy-on-write]
        P[Process PID 1<br/>CMD/ENTRYPOINT]
    end
...
Vì vậy, tối ưu Dockerfile không chỉ là giảm dung lượng. Nó còn quyết định cache build, tốc độ pull, bề mặt tấn công và khả năng tái tạo của artifact.
